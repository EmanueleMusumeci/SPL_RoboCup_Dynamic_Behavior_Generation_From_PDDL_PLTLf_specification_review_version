...
def setup_experiment():
    ...
    domain_path = os.path.join(currentdir, "PDDL", "simple_striker_domain.pddl")
    problem_path = os.path.join(currentdir, "PDDL", "simple_striker_problem.pddl")
    #striker_mapping_path = os.path.join(currentdir, "PDDL", "simple_striker_mapping.map")
    striker_mapping_path = None
    working_dir = os.path.join(currentdir, "output")

    simple_approacher_pltlf_goal = "isat_ball_goaltarget && O(isat_ball_kickingposition)"
    print("Creating Policy for striker role from domain file: %s with problem file %s and PLTLf formula '%s'\n" % (domain_path, problem_path, simple_approacher_pltlf_goal))
    try:
        simple_approacher_plan = Policy.build_from_FOND_PDDL_and_PLTLf_formula(domain_path, problem_path, working_dir, simple_approacher_pltlf_goal, mapping_path = striker_mapping_path)
