(define (domain robocupdeterministic)
    ...
    (:predicates (balldribbledto) (ballkickedto) (goalscored) (isat ?mov ?loc) (isball ?mov) (isrobot ?mov) (movedto) (precedes) (val_isat_ball_goaltarget))
    ...
    (:action carryball
        ...
        :effect (and (and (not (isat ?rob ?from)) (isat ?rob ?to) (not (isat ?b ?from)) (isat ?b ?to) (balldribbledto ?to)))
    )
     (:action kickball
        :parameters (?rob ?b ?from ?to)
        :precondition (and (isball ?b) (isrobot ?rob) (isat ?rob ?from) (not (isat ?rob ?to)) (isat ?b ?from) (not (isat ?b ?to)) (or (precedes ?from ?to) (initialposition ?from)))
        :effect (and (and (isat ?rob ?from) (not (isat ?b ?from)) (isat ?b ?to) (ballkickedto ?to)))
    )
     (:action moverobot
        :parameters (?rob ?from ?to)
        :precondition (and (isrobot ?rob) (isat ?rob ?from) (not (isat ?rob ?to)))
        :effect (and (and (not (isat ?rob ?from)) (isat ?rob ?to) (movedto ?to)))
    )
)