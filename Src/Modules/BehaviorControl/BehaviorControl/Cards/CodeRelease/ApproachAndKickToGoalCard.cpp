...
CARD(ApproachAndKickToGoalCard,
{,
  CALLS(InWalkKick),
  CALLS(LookForward),
  CALLS(LookAtPoint),
...
class ApproachAndKickToGoalCard : public ApproachAndKickToGoalCardBase
{
    ...
    return theTaskController.getCurrentActionType() != HRI::ActionType::CarryAndKickToGoal
            ...
            (
              ...
              theBallModel.estimate.position.norm() > theBallCarrierModel.maximumApproachDistance) 
                    && 
                    option_time > optionOscillationTime
                        //&& !isSearchingToggle 
            )
            ||
            !theFieldBall.ballWasSeen(ballNotSeenTimeout)
            ; //NOW SEARCHING HAPPENS IN THE BALL CARRIER
  }
...
//NOTICE: the robot hitting the ball with the side of his foot is caused by this
  ...
  Vector2f chooseGoalTarget()
  {
    ...
    return theLibCheck.goalTarget(false, true);
  }

};

MAKE_CARD(ApproachAndKickToGoalCard);
