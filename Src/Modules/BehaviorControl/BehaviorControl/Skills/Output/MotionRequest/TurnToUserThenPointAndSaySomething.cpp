/**
 * @file TurnToUserThenPointAndSaySomething.cpp
 *
 * This skill turns the robot's head to the user, makes it point at a position with its arm and plays an audio file
 *
...
SKILL_IMPLEMENTATION(TurnToUserThenPointAndSaySomethingImpl,
{,
  IMPLEMENTS(TurnToUserThenPointAndSaySomething),
  CALLS(Activity),
  CALLS(InWalkKick),
  CALLS(LookForward),
  CALLS(LookAtPoint),
...
class TurnToUserThenPointAndSaySomethingImpl : public TurnToUserThenPointAndSaySomethingImplBase
{
    bool soundPlaying = false;

    option(TurnToUserThenPointAndSaySomething)
    {
        initial_state(start)
        {
            std::cout<<"TurnToUserThenPointAndSaySomethingSkill: start"<<std::endl;
            transition
            {
                if(state_time > initialWaitTime)
                goto turnHeadToUserAndPoint;
            }
        ...
        state(turnHeadToUserAndPoint)
        {
            transition
            {
                if(state_time > waitBeforePlayingSound)
                {
                    std::cout<<"turnHeadToUserAndPoint -> speakToHuman: TIMEOUT"<<std::endl;
                    goto speakToHuman;
                }
            }
            ...
            action
            {
                ...
                thePointAtSkill(Vector3f(localTarget.x(), localTarget.y(), 0.f));
...
MAKE_SKILL_IMPLEMENTATION(TurnToUserThenPointAndSaySomethingImpl);